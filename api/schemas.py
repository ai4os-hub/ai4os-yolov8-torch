...
class PredArgsSchema(marshmallow.Schema):
    ...
    input = fields.Field(
        ...
        type="file",
        location="form",
        metadata={'description': "Input an image or Video.\n"
    ...
    model = fields.Str(
        metadata= {'description':"The timestamp when you saved your trained model,"
    ...
    task_type = fields.Str(
        metadata= {'description':"The type of task for load the pretrained model:\n"
    ...
    iou = fields.Float(
        metadata= {'description':"intersection over union (IoU) threshold for NMS",
        },
        load_default= 0.5
    )

    show_labels = fields.Boolean(
        metadata= {'description':"Show object labels in plots",
        },
        load_default= True
    )
    show_conf = fields.Boolean(
        metadata= {'description':"Show object confidence scores in plots."
    ...
    augment = fields.Boolean(
        metadata= {'description':"Apply image augmentation to prediction sources",
        },
        load_default= False
    )
    classes = fields.Field(
        metadata= {'description':"Filter results by class, i.e. class=0, or class=[0,2,3]"},
        load_default= None,
...
class TrainArgsSchema(marshmallow.Schema):
    ...
    task_type = fields.Str(
        metadata= {'description':"The type of task for the model:\n"
        '"det" for object detection model\n'
        '"seg" for object segmentation model\n'
        '"cls" for object classification model\n'
        'The default is "det"',
    ...
    model = fields.Str(  # FIXME
        metadata= {'description':" name of the model to train\n"
    ...
    data = fields.Str(
        metadata= {'description':".pytest_cache/Path to the config data file, e.g.,"
    ...
    epochs = fields.Int(
        metadata= {'description':"Number of epochs to train for",
        },
        load_default= 100
    )
    patience = fields.Int(
        metadata= {'description':"Epochs to wait for no observable improvement for"
        " early stopping of training"},
        load_default= 10,
    )
    batch = fields.Int(
        metadata= {'description':"Number of images per batch (-1 for AutoBatch)",
        },
        load_default=2
    )
    imgsz = fields.Int(
        metadata= {'description':"Input images size as int for train and val modes,"
        " or list [w, h] for predict and export modes"},
        load_default= 640
    )
    weights = fields.Str(
        metadata= {'description':"If you want to initialize weights for training"
    ...
    resume = fields.Bool(
        metadata= {'description':"If the training was stopped before completing all"
    ...
    save_period = fields.Int(
        metadata= {'description':"Save checkpoint every x epochs (disabled if < 1)",
        },
        load_default=-1
    )

    device = fields.Str(
        metadata= {'description':'Device to run on, e.g., "cuda:0" or "cpu"',
        },
        load_default= "cuda:0"
    )
    workers = fields.Int(
        metadata= {'description':"Number of worker threads for data loading"
        " (per RANK if DDP)",
        },
        load_default= 4
    )
    ...
    optimizer = fields.Str(
        metadata= {'description':"Optimizer to use, choices="
    ...
    deterministic = fields.Bool(
        metadata= {'description':"Whether to enable deterministic mode",
        },
        load_default=True,
         enum=  [True, False],
    )
    single_cls = fields.Bool(
        metadata= {'description':"Train multi-class data as single-class",
        },
        load_default=False,
        enum=  [True, False],
    )
    rect = fields.Bool(
        metadata= {'description':"Rectangular training (mode='train') or rectangular"
        " validation (mode='val')",
        },
        load_default= False,
        enum=  [True, False]
    ...
    fraction = fields.Float(
        metadata= {'description':"Dataset fraction to train on (default is 1.0,"
    ...
    mask_ratio = fields.Int(
        metadata= {'description':"Mask downsample ratio (segment train only)",
        },
        load_default= 4
    )
    dropout = fields.Float(
        metadata= {'description':"Use dropout regularization (classify train only)",
        },
        load_default=0.0
    )
    amp = fields.Bool(
        metadata= {'description':"Automatic Mixed Precision (AMP) training,"
    ...
    cos_lr = fields.Bool(
        metadata= {'description':"Use cosine learning rate scheduler",
        },
        load_default=False,
         enum= [True, False]
    )
    lr0 = fields.Float(
        metadata= {'description':"Initial learning rate (i.e. SGD=1E-2, Adam=1E-3)",
        },
        load_default=0.01
    )
    lrf = fields.Float(
    ...
    momentum = fields.Float(
        metadata= {'description':"SGD momentum/Adam beta1",
        },
        load_default=0.937
    )
    ...
    warmup_epochs = fields.Float(
        metadata= {'description':"Warmup epochs (fractions ok)",
        },
        load_default=3.0
    )
    warmup_momentum = fields.Float(
        metadata= {'description':"Warmup initial momentum",
        },
        load_default=0.8
    )
    warmup_bias_lr = fields.Float(
        metadata= {'description':"Warmup initial bias lr",
        },
        load_default=1.0
    )
    close_mosaic = fields.Int(
        metadata= {'description':"Disable mosaic augmentation for final epochs",
        },
        load_default=10
    )

    box = fields.Float(
        metadata= {'description':"Box loss gain"},
         load_default=7.5
    )
    cls = fields.Float(
        metadata= {'description':"Cls loss gain (scale with pixels)",
        
        },
        load_default=0.5
    )
    dfl = fields.Float(
        metadata= {'description':" Distribution Focal Loss gain",
    ...
    kobj = fields.Float(
        metadata= {'description':"Keypoint obj loss gain",
    ...
    label_smoothing = fields.Float(
        metadata= {'description':"Label smoothing (fraction)",
    ...
    hsv_h = fields.Float(
        metadata= {'description':"Image HSV-Hue augmentation (fraction)"},
        
        load_default=0.015
    )
    hsv_s = fields.Float(
        metadata= {'description':"Image HSV-Saturation augmentation (fraction)"},
        
        load_default=0.7
    )
    hsv_v = fields.Float(
        metadata= {'description':"Image HSV-Value augmentation (fraction)"},
    ...
    perspective = fields.Float(
        metadata= {'description':"Image perspective (+/- fraction), range 0-0.001"},
    ...
    fliplr = fields.Float(
        metadata= {'description':"Image flip left-right (probability)"},
