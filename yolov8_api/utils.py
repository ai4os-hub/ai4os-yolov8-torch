...
def mlflow_update():
    ...
    client.set_registered_model_tag(
        MLFLOW_MODEL_NAME, "task", "detection"
    )
    client.set_registered_model_tag(
        MLFLOW_MODEL_NAME, "author", cfg.MLFLOW_AUTHOR
    )
    client.set_registered_model_tag(
        MLFLOW_MODEL_NAME, "framework", "pytorch"
    )
...
def mlflow_logging(model, num_epochs, args):
    ...
    with mlflow.start_run(run_name=args["name"]):
        ...
        run.log_params(
            {**vars(model.trainer.model.args), **get_git_info()}
        )
        ...
        for result in prediction_inf:
            ...
            output_file_path = os.path.join(
                config.MODELS_PATH, "detection_results.txt"
            )
